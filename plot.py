...
def plot_base(w,h,dpi,limits):
        ...
        plt.axis(limits, crs=ccrs.PlateCarree())
        return(fig)

def plot_temp(i, tmp2m, levels, cmap1, times, fig):
        # Formato de las etiquetas de los ejes (ticks)
        #plt.rc('font', weight='bold', size=8)

        # Hora de salida
        fecha=times.tobytes().decode('utf-8')
...
lon_e=-74.876724
lon_o=-123.36128
limits=[lon_o,lon_e,lat_s,lat_n]
print('ROI:',lat_n, lat_s, lon_e, lon_o)
